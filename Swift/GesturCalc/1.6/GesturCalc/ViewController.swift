...
class ViewController: UIViewController {
    ...
    */
    ...
    @IBAction func removeLastDigit(_ sender: AnyObject) {
        guard var labelText = label.text else { return }
        ...
        if (labelText.count) > 1 {
            if labelText.last == "." {
                ...
                labelText.removeLast()
                guard let labelTextDouble = Double(labelText) else { return }
                label.text = formatForLabel(labelTextDouble)
                guard let numberOnScreen = Double(labelText.replacingOccurrences(of: ",", with: "")) else { return }
    ...
    func showPopup(_ operation: String) {
        popupLabel.text = operation
